...
## 4. Infrastructure Agents (Non-Paired)

**Some agents operate outside the worker-critic paradigm. They provide logging, orchestration, or verification services.**

| Agent | Role | Trigger | Model |
|-------|------|---------|-------|
| orchestrator | Phase transitions, dispatch, scoring | Automatic after plan approval | inherit |
| verifier | Compilation, execution, output checks | Phase transitions, pre-commit | inherit |
| stenographer | Raw action recording — logs what, not why | After every completed TODO item | haiku |

### Stenographer Protocol

The stenographer is a **lightweight, fast recorder** (runs on Haiku for speed and cost):

- **Fires after every completed TODO item** — appends a timestamped entry to the active session log
- **Records only facts:** files modified, commands run, pass/fail outcomes
- **Never interprets:** "Edited lines 158-174" not "Improved the paragraph"
- **Never self-documents:** completing a stenographer entry does NOT trigger another stenographer entry
- **3-5 lines per entry**, always includes file paths and command outcomes
- **Distinct from session log:** stenographer records the *what*; session log provides the *why*

See `.claude/agents/stenographer.md` for full specification.
