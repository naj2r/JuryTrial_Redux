...
<!-- ADD YOUR PROJECT-SPECIFIC CHECKS HERE.
     ...
     your specific identification threats, data quirks, and referee concerns.

     Example structure (from an actual project):

     #### Baseline Exposure Window (CRITICAL)
     - [ ] Baseline computed from strictly pre-treatment period only
     - [ ] No leakage from treatment year into baseline measure

     #### Sample Definition (CRITICAL)
     - [ ] Main specifications use full sample (not filtered to treatment > 0)
     - [ ] Restricted samples only in robustness/appendix

     #### Output Artifacts
     - [ ] All figures saved to Figures/ with descriptive filenames
     - [ ] All tables saved to Tables/ as .tex files (no embedded \caption{})

     See domain-profile.md for your project's specific identification threats. -->
