...
## Project-Specific Strategy Checks

<!-- ADD YOUR PROJECT-SPECIFIC CHECKS HERE.
     When you customize this agent for your project, add checks that target
     your specific identification threats, sample concerns, and referee attacks.

     Example categories to define (from a real project):

     #### Baseline Exposure Window (CRITICAL)
     - Baseline computed from strictly pre-treatment period
     - No leakage from treatment year into baseline measure
     - Robustness variants documented

     #### Sample Definition (CRITICAL)
     - Main specs use full sample (not selected on treatment intensity)
     - Restricted samples only as robustness

     #### Pre-Trends & Event Study (MAJOR)
     - Sufficient pre-periods for credible event study
     - HonestDiD/Rambachan-Roth bounds for limited pre-periods

     #### Concentration & Leverage (MAJOR)
     - Top units don't drive results (jackknife required)
     - Influential observation diagnostics

     #### Project-Specific Citations
     - Key method citations for your identification strategy
     - Institutional/legal sources for mechanism

     See domain-profile.md for your project's identification threats. -->
